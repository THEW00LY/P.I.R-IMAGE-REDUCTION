...
SIZE_ALGOS = {
    "Lanczos": {
        "script": os.path.join("algos", "lanczos.py"),
        "output": os.path.join("output_images", "output_lanczos.jpg"),
        "params": [
    ...
    "Discret Cosine Transform": {
        "script": os.path.join("algos", "dct.py"),
        "output": os.path.join("output_images", "output_dct.jpg"),
        "params": [
            {"label": "Quality", "default": 10, "min": 1, "key": "q"},
        ],
    },
}
...
COLOR_ALGOS = {
    "Octree": {
        "script": os.path.join("algos", "octree.py"),
        "output": os.path.join("output_images", "output_octree.png"),
        "params": [
            {"label": "Nb couleurs", "default": 32, "min": 8, "type": "number"},
        ],
    },
    "Median Cut": {
        "script": os.path.join("algos", "median-cut.py"),
        "output": os.path.join("output_images", "output_mediancut.png"),
        "params": [
    ...
    "Palette définie": {
        "script": os.path.join("algos", "definedPalet.py"),
        "output": os.path.join("output_images", "output_definedpalette.png"),
        "params": [],
    },
}
...
defined_palette_widgets = []
defined_palette_rows = []
defined_palette_controls = None
defined_palette_colors_container = None
defined_palette_count_label = None


def update_defined_palette_label():
    if defined_palette_count_label is not None:
        defined_palette_count_label.set_text(f"Couleurs : {len(defined_palette_widgets)}")


def add_defined_palette_color(default='#ffffff'):
    global defined_palette_widgets, defined_palette_rows, defined_palette_colors_container
    if defined_palette_colors_container is None:
        return
    if len(defined_palette_widgets) >= 100:
        ui.notify("Maximum 100 couleurs atteint", type='negative')
        return
    with defined_palette_colors_container:
        row = ui.row().classes('items-center gap-3')
        with row:
            color_input = ui.input(value=default).props('type=color').classes('w-36')
            ui.button(icon='close').props('flat round').classes('text-red').on_click(
                lambda r=row: remove_color_by_row(r)
            )
    defined_palette_widgets.append(color_input)
    defined_palette_rows.append(row)
    algo_widgets['Palette définie'] = defined_palette_widgets
    update_defined_palette_label()


def remove_color_by_row(row):
    global defined_palette_widgets, defined_palette_rows
    if len(defined_palette_widgets) <= 1:
        ui.notify("Il faut au moins une couleur", type='negative')
        return
    try:
        index = defined_palette_rows.index(row)
        defined_palette_widgets.pop(index)
        defined_palette_rows.pop(index)
        row.delete()
...
def remove_defined_palette_color():
    global defined_palette_widgets, defined_palette_rows
    if len(defined_palette_widgets) <= 1:
        ui.notify("Il faut au moins une couleur", type='negative')
        return
    defined_palette_widgets.pop()
    row = defined_palette_rows.pop()
    try:
        row.delete()
    except Exception:
        pass
...
def build_defined_palette_controls():
    global defined_palette_widgets, defined_palette_rows, defined_palette_controls, defined_palette_colors_container, defined_palette_count_label
    defined_palette_widgets = []
    defined_palette_rows = []
    defined_palette_controls = ui.column().classes('gap-3')
    with defined_palette_controls:
        defined_palette_count_label = ui.label(f"Couleurs : {len(defined_palette_widgets)}").classes('text-caption')
        with ui.row().classes('gap-2'):
            ui.button('Ajouter une couleur', on_click=lambda e: add_defined_palette_color()).classes('px-3')
        defined_palette_colors_container = ui.column().classes('gap-2')
    with defined_palette_colors_container:
        add_defined_palette_color('#ff0000')
        add_defined_palette_color('#00ff00')
        add_defined_palette_color('#0000ff')
        add_defined_palette_color('#ffff00')
        add_defined_palette_color('#00ffff')
...
app.add_static_files('/output_images', OUTPUT_DIR)
ui.add_head_html('<link rel="stylesheet" href="/static/GUI_Style.css">')
ui.dark_mode(False)
...
def get_param_values(algo_name):
    values = []
    for w in algo_widgets.get(algo_name, []):
        value = w.value
...
def build_param_widgets(algo_name, params):
    ...
    for p in params:
        ...
        elif p.get("type") == "color":
            w = ui.input(label=p["label"], value=p["default"]).props('type=color').classes('w-36')
        elif key == "w":
...
def on_color_change(value):
    ...
    if value and value in COLOR_ALGOS and value != '— aucun —':
        if value == 'Palette définie':
            with color_params_container:
                build_defined_palette_controls()
        else:
            with color_params_container:
